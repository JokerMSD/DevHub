...
export const CreateTechModal = ({ headerText, buttonText }) => {
  const [techName, setTechName] = useState("");
  const [techStatus, setTechStatus] = useState("");

  const { loading, isModalOpen, handleCloseModal, createTech } =
    ...
    <>
      <section
        ...
        className={`${Style.modalSection} ${
          isModalOpen ? Style.fadein : Style.hidden
        }`}
      ...
      >
        ...
        <div onClick={handleCloseModal} className={Style.backdrop}></div>
      </section>
    </>
  );
};
